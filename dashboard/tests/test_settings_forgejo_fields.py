...
"""Task 9 added FORGEJO_INSTANCE_URL, FORGEJO_ADMIN_TOKEN and
...
FORGEJO_ADMIN_TOKEN mints registration tokens and deletes runners, so it is
...
def test_all_three_forgejo_fields_are_present():
    html = _html()
    for key in ("FORGEJO_INSTANCE_URL", "FORGEJO_ADMIN_TOKEN", "FORGEJO_RUNNER_LABELS"):
        assert f'id="{key}"' in html, key


def test_forgejo_admin_token_gets_the_same_password_treatment_as_gh_token():
    html = _html()
    # Same input type as GH_TOKEN: the real value is never rendered.
    assert re.search(r'<input[^>]*type="password"[^>]*id="FORGEJO_ADMIN_TOKEN"', html), \
        "FORGEJO_ADMIN_TOKEN must be a password input, like GH_TOKEN"
...
def test_forgejo_admin_token_box_is_cleared_after_save_like_gh_token():
    """GH_TOKEN's box is blanked after a successful save so a just-typed
    secret is not left sitting in the DOM. FORGEJO_ADMIN_TOKEN must get the
    identical treatment."""
    html = _html()
    assert "$('GH_TOKEN').value = ''" in html
    assert "$('FORGEJO_ADMIN_TOKEN').value = ''" in html
